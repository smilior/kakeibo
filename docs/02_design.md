...
COMMENT ON COLUMN expenses.is_family IS '家族の支出フラグ（true: 家族, false: 個人）';
...
| S02 | セットアップ | `/setup` | 家計作成または招待リンク入力 |
| S03 | ダッシュボード | `/` | メイン画面 |
| S04 | 支出入力 | `/expenses/new` | 支出登録フォーム |
| S05 | 支出履歴 | `/expenses` | 月別支出一覧（編集・削除可） |
| S06 | 分析 | `/analytics` | グラフ・統計 |
| S07 | サブスク管理 | `/subscriptions` | サブスク一覧・登録 |
| S08 | 設定 | `/settings` | 設定メニュー |
| S09 | カテゴリ管理 | `/settings/categories` | カテゴリCRUD |
| S10 | ルール設定 | `/settings/rules` | 回数ルール設定 |
| S11 | 家計設定 | `/settings/household` | 家計情報・招待・締め日設定 |
| S12 | LINE連携 | `/settings/line` | LINE設定（サービス終了） |
...
> **⚠️ 重要な注意事項**
> LINE Notifyは2025年3月31日でサービスを終了しました。
> 今後LINE連携を実装する場合は、LINE Messaging APIを使用する必要があります。
> 現在のアプリではLINE通知機能は無効化されています。

### 9.1.1 LINE Notify連携（サービス終了）
...
### 9.1.2 LINE Notify トークン発行手順（サービス終了・参考情報）

> **注意:** 以下はLINE Notifyサービス終了前の手順です。現在は利用できません。
...
### 9.2 LINE Messaging API連携（将来実装予定）

LINE Messaging APIを使用した通知機能の実装は将来の拡張として検討中です。
実装時には以下の要素が必要になります：
...
export type Database = {
  public: {
    Tables: {
      ...
      // ... 他のテーブル（categories, rules, subscriptions, invitations）も同様
    }
  }
}
```
